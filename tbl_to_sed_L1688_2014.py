...
Abstract:
    ...
    in L1688 2014 region in IPAC Infrared Science Archive (IRSA) 
    to AI model readable format. 
Usage:
    tbl_to_sed_L1688_2014.py [L1688 2014 table]
Output:
...
if __name__ == "__main__":
    ...
    if len(argv) != 2:
        ...
        print ("tbl_to_sed_L1688_2014.py [L1688 2014 table]")
        exit()
    ...
    # Load Quality flags
    J2Q =  limarray_to_Qarray(inp_table[:, 13])
    H2Q =  limarray_to_Qarray(inp_table[:, 16])
    K2Q =  limarray_to_Qarray(inp_table[:, 19])
    IR1Q = limarray_to_Qarray(inp_table[:, 22])
    IR2Q = limarray_to_Qarray(inp_table[:, 25])
    IR3Q = limarray_to_Qarray(inp_table[:, 28])
    IR4Q = limarray_to_Qarray(inp_table[:, 31])
    MP1Q = limarray_to_Qarray(inp_table[:, 34])
    ...
    H2mag  = np.array(np.transpose([inp_table[:,17], inp_table[:,18]]), dtype = float) 
    K2mag  = np.array(np.transpose([inp_table[:,20], inp_table[:,21]]), dtype = float) 
    IR1mag = np.array(np.transpose([inp_table[:,23], inp_table[:,24]]), dtype = float) 
    IR2mag = np.array(np.transpose([inp_table[:,26], inp_table[:,27]]), dtype = float) 
    IR3mag = np.array(np.transpose([inp_table[:,29], inp_table[:,30]]), dtype = float) 
    IR4mag = np.array(np.transpose([inp_table[:,32], inp_table[:,33]]), dtype = float) 
    MP1mag = np.array(np.transpose([inp_table[:,35], inp_table[:,36]]), dtype = float) 
    #-----------------------------------
    ...
    source_type = np.array(inp_table[:, 51])
    source_type[source_type == '0'] = 'null'
    source_type[source_type == '1'] = 'YSO'
    #-----------------------------------
    # Save the data
    print ('Save the data')
    np.savetxt( 'L1688_2014_sed.txt', 
                flux_sed)
    np.savetxt( 'L1688_2014_coord.txt', 
                coord, 
                fmt = '%s')
    np.savetxt( 'L1688_2014_Av.txt', 
                fake_Av, 
                header = '# fake')
    np.savetxt( 'L1688_2014_Q.txt', 
                Q_array, 
                fmt = '%s')
    np.savetxt( 'L1688_2014_Sp.txt',
                source_type,
                fmt = '%s'
                )
    #-----------------------------------
