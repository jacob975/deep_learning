...
Usage:
    exclude_XU_678.py [Q flag table] [data]
...
if __name__ == "__main__":
    ...
    if len(argv) != 3:
        ...
        print ("Usage: exclude_XU_678.py [Q flag table] [data]")
        exit(0)
    Q_table_name = argv[1]
    data_name = argv[2]
    #-----------------------------------
...
#                            (Q_table[:,4] != 'X') &\
    ...
    np.savetxt("{0}_exXU_678.txt".format(data_name[:-4]), data_woX_woU, fmt = '%s')
    np.savetxt("index_exXU_678.txt", index_woX_woU, fmt = '%d')
    #-----------------------------------
    # measure time
