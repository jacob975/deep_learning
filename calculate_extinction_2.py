...
Usage:
    calculate_extinction_2.py [coord_table] [2mass sed table] [Rv] [bin size]
...
if __name__ == "__main__":
    ...
    if len(argv) != 5:
        ...
        print ("Usage: calculate_extinction_2.py [coord_table] [mag_table] [Rv] [bin size in arcmin]")
        print ("Available Rv: WD55B, WD31B")
        exit(1)
    coord_table_name = argv[1]
    mag_table_name = argv[2]
    Rv = argv[3]
    bin_size = float(argv[4])/60.
    #--------------------------------------------
    # Load files
    ...
    science_ensemble_mag = np.loadtxt(mag_table_name, dtype = float)
    science_mag = science_ensemble_mag[:,:3] 
    science_err_mag = science_ensemble_mag[:,3:] 
