...
'''
Abstract:
    This is a program for convert the SED data of SWIRE Lockman survey (2005) to AI model readable format. 
Usage:
    tbl_to_sed_Lockman_2005.py [Lockman 2005 table]
Output:
...
if __name__ == "__main__":
    ...
    if len(argv) != 2:
        ...
        print ("tbl_to_sed_Lockman_2005.py [Lockman 2005 table]")
        exit()
    ...
    #-----------------------------------
    # Save the data
    np.savetxt('Lockman_sed.txt', flux_sed)
    np.savetxt('Lockman_coord.txt', coord, fmt = '%s')
    np.savetxt('Lockman_c2d2007_Sp.txt', source_type, header = '# fake source type')
    np.savetxt('Lockman_Av.txt', fake_Av, header = '# fake Av')
    np.savetxt('Lockman_Q.txt', fake_Q, fmt = '%s', header = '# fake quality label')
    #-----------------------------------
    # Measure time
