...
Abstract:
    ...
    in NGC1333 2014 region in IPAC Infrared Science Archive (IRSA) 
    to AI model readable format. 
Usage:
    tbl_to_sed_NGC1333_2014.py [NGC1333 2014 table]
Output:
...
if __name__ == "__main__":
    ...
    if len(argv) != 2:
        ...
        print ("tbl_to_sed_NGC1333_2014.py [NGC1333 2014 table]")
        exit()
    ...
    # Load Quality flags
    J2Q =  limarray_to_Qarray(inp_table[:, 15])
    H2Q =  limarray_to_Qarray(inp_table[:, 18])
    K2Q =  limarray_to_Qarray(inp_table[:, 21])
    IR1Q = limarray_to_Qarray(inp_table[:, 24])
    IR2Q = limarray_to_Qarray(inp_table[:, 27])
    IR3Q = limarray_to_Qarray(inp_table[:, 30])
    IR4Q = limarray_to_Qarray(inp_table[:, 33])
    MP1Q = limarray_to_Qarray(inp_table[:, 36])
    ...
    J2mag  = np.array(np.transpose([inp_table[:,16], inp_table[:,17]]), dtype = float) 
    H2mag  = np.array(np.transpose([inp_table[:,19], inp_table[:,20]]), dtype = float) 
    K2mag  = np.array(np.transpose([inp_table[:,22], inp_table[:,23]]), dtype = float) 
    IR1mag = np.array(np.transpose([inp_table[:,25], inp_table[:,26]]), dtype = float) 
    IR2mag = np.array(np.transpose([inp_table[:,28], inp_table[:,29]]), dtype = float) 
    IR3mag = np.array(np.transpose([inp_table[:,31], inp_table[:,32]]), dtype = float) 
    IR4mag = np.array(np.transpose([inp_table[:,34], inp_table[:,35]]), dtype = float) 
    MP1mag = np.array(np.transpose([inp_table[:,37], inp_table[:,38]]), dtype = float) 
    #-----------------------------------
    ...
    source_type = np.array(inp_table[:, 49])
    source_type[source_type == '0'] = 'null'
    source_type[source_type == '1'] = 'YSO'
    #-----------------------------------
    # Save the data
    print ('Save the data')
    np.savetxt( 'NGC1333_2014_sed.txt', 
                flux_sed)
    np.savetxt( 'NGC1333_2014_coord.txt', 
                coord, 
                fmt = '%s')
    np.savetxt( 'NGC1333_2014_Av.txt', 
                fake_Av, 
                header = '# fake')
    np.savetxt( 'NGC1333_2014_Q.txt', 
                Q_array, 
                fmt = '%s')
    np.savetxt( 'NGC1333_2014_Sp.txt',
                source_type,
                fmt = '%s'
                )
    #-----------------------------------
