...
def get_quality_flag(flux, upper):
    num_row = len(flux)
...
if __name__ == "__main__":
    ...
    # Load band-fill data from input table
    ...
    IR1Q = get_quality_flag(IR1flux[:,0], IR1upper[:,0]) 
    IR2Q = get_quality_flag(IR2flux[:,0], IR2upper[:,0]) 
    IR3Q = get_quality_flag(IR3flux[:,0], IR3upper[:,0]) 
    IR4Q = get_quality_flag(IR4flux[:,0], IR4upper[:,0]) 
